<template>
  <div class="root">
      ...
      <div class="container rel" style="--max-width: 632px">
        ...
        <div :class="['board', 'hard-shadow']" :key="renderKey">
          <template v-for="(row, row_index) in board" :key="row_index">
            <div :class="['cell']" v-for="(col, col_index) in row" :key="`${row_index}, ${col_index}`"
                 :data-row="row_index" :data-col="col_index" @click="putHere(col_index)" @mouseover="setRadPos"
        ...
        <div :class="['arrow', playerColor]" v-if="!winner && !draw">
          ...
          <div class="content">
            ...
            <div class="time">{{ secondsLeft }}s</div>
          </div>
        </div>
    ...
    <CFModal :isOpen="isMenuOpen" :overlay="true" @close="resume" h="49rem">
      <div class="menu">
        <h1>Pause</h1>
        <div class="actions">
          <ButtonComponent label="Continue Game" @click="resume"></ButtonComponent>
          <ButtonComponent label="Restart" @click="
            reset(true);
            isMenuOpen = false;
          "></ButtonComponent>
...
const {putHere, winner, draw, getAvailableCols, getLastEmptyCellRow} =
    useMoveCoinThrough(
        reactive({
          activePlayer,
          board,
    ...
    async () => {
      if (activePlayer.value === 2 && vsCPU) {
        ...
        setTimeout(async () => {
          ...
          else onHold.value = secondsLeft.value > 2
...
function resetTimer() {
  if (!winner.value && !isMenuOpen.value) {
    clearInterval(timer);
    playingState.setTimeRemaining(15 * 1000);
    timer = setInterval(() => {
      playingState.setTimeRemaining(
          secondsLeft.value > 0 ? playingState.timeRemaining - 1000 : 0
      );
    }, 1000);
  }
}
